-- SAFE DATABASE SETUP - Phase 4: Functions and Triggers (FIXED)
-- Run this LAST, after Phases 1-3 complete successfully

-- ===========================================
-- PHASE 4: CREATE FUNCTIONS AND TRIGGERS (FIXED VERSION)
-- ===========================================
...
DROP TRIGGER IF EXISTS update_revenue_types_updated_at ON public.revenue_types;
...
DROP TRIGGER IF EXISTS log_payment_status_changes ON public.payments;
CREATE TRIGGER log_payment_status_changes
AFTER UPDATE ON public.payments
FOR EACH ROW EXECUTE FUNCTION public.log_payment_status_change();

-- ===========================================
-- ENABLE REALTIME (SAFE VERSION - Don't duplicate payments)
-- ===========================================
-- Only add tables that aren't already in the publication
DO $$
BEGIN
    -- Check if receipts is already in publication
    IF NOT EXISTS (
    ...
    -- Check if reminders is already in publication
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
        WHERE pubname = 'supabase_realtime'
    ...
    -- Check if reconciliation_log is already in publication
    IF NOT EXISTS (
        SELECT 1 FROM pg_publication_tables
        WHERE pubname = 'supabase_realtime'
